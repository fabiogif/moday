...
export function StoreHoursBanner({ slug, onStatusChange, className }: StoreHoursBannerProps) {
  ...
  if (!canExpand) {
    return (
      <button type="button" disabled className={cn('inline-block cursor-default', className)}>
        {badge}
      </button>
    )
  }
  ...
  return (
    <Popover open={expanded} onOpenChange={setExpanded}>
      ...
      <PopoverContent
        align="start"
        collisionPadding={16}
        className="w-72 max-w-[calc(100vw-2rem)] rounded-xl p-3 text-sm"
      >
          ...
          <div className="space-y-1 text-xs">
            {Object.entries(allHours).map(([day, hours]) => (
              <div key={day} className="flex items-baseline justify-between gap-3">
                ...
                <span className="whitespace-nowrap text-muted-foreground">{formatSlots(hours, ', ')}</span>
              </div>
            ))}
          </div>
        </div>
